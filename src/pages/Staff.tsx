...
import { UserCog, Search, Plus, Edit2, Eye } from 'lucide-react';
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">Manage hospital staff and personnel records</p>
        </div>
        <Button icon={Plus}>
          Add Staff Member
        </Button>
        ...
        <DataTable
          ...
          columns={[
            ...
            { header: 'Name', accessor: 'name' },
            {
              header: 'Role',
              accessor: (row) => (
                <span className={`px-3 py-1 rounded-full text-xs font-medium ${getRoleBadgeColor(row.role)}`}>
                  {row.role.replace('_', ' ')}
                </span>
              ),
            },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                ...
                <span className="font-bold text-gray-900">{item.count} staff</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white/60 backdrop-blur-md rounded-xl shadow-soft p-6 border border-gray-100">
          <h2 className="text-xl font-bold text-gray-900 mb-4">Upcoming Leave Requests</h2>
          <div className="space-y-3">
            {[
              { name: 'Dr. Emily Brown', dates: 'Nov 1-5, 2024', type: 'Vacation' },
              { name: 'Nurse Sarah Lee', dates: 'Nov 8-10, 2024', type: 'Medical' },
              { name: 'Dr. James Wilson', dates: 'Nov 15-20, 2024', type: 'Conference' },
            ].map((item, index) => (
              <div key={index} className="p-3 bg-gray-50 rounded-lg">
                <div className="flex items-center justify-between mb-1">
                  <p className="font-medium text-gray-900">{item.name}</p>
                  <span className="text-xs px-2 py-1 bg-blue-100 text-blue-700 rounded-full">{item.type}</span>
                </div>
