import { UserPlus, Calendar, DollarSign, Users, Search, Clock } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-display font-bold text-gray-900">Reception Dashboard</h1>
          <p className="text-gray-600 mt-1">Manage patient registration and appointments</p>
        </div>
        <div className="flex gap-3">
          <Button variant="outline" icon={Calendar}>Schedule Appointment</Button>
          <Button variant="primary" icon={UserPlus}>Register Patient</Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white/60 backdrop-blur-md rounded-xl shadow-soft p-6 border border-gray-100">
          <h2 className="text-xl font-bold text-gray-900 mb-4">Appointment Queue</h2>
        ...
        <div className="bg-white/60 backdrop-blur-md rounded-xl shadow-soft p-6 border border-gray-100">
          ...
          <div className="space-y-3">
            {[1, 2, 3].map((i) => (
              <div key={i} className="p-4 bg-warning-50 border border-warning-200 rounded-lg">
                <div className="flex items-center justify-between mb-2">
                  <p className="font-medium text-gray-900">Patient #{1000 + i}</p>
                  <span className="font-bold text-warning-600">${(Math.random() * 500 + 100).toFixed(2)}</span>
                </div>
                <div className="flex gap-2">
                  <Button size="sm" variant="primary" className="flex-1">Collect Payment</Button>
      ...
      <div className="bg-white/60 backdrop-blur-md rounded-xl shadow-soft p-6 border border-gray-100">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {['Dr. Smith', 'Dr. Johnson', 'Dr. Williams'].map((doctor, i) => (
            <div key={i} className="p-4 bg-success-50 border border-success-200 rounded-lg">
