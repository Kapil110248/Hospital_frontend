import { DollarSign, TrendingUp, CreditCard, FileText, Users, Calendar } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-display font-bold text-gray-900">Finance Dashboard</h1>
          <p className="text-gray-600 mt-1">Revenue, billing, and financial analytics</p>
        </div>
        <div className="flex gap-3">
          <Button variant="outline" icon={FileText}>Generate Report</Button>
          <Button variant="primary" icon={CreditCard}>Process Payment</Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2">
          <div className="bg-white/60 backdrop-blur-md rounded-xl shadow-soft p-6 border border-gray-100 h-full">
            <h2 className="text-xl font-bold text-gray-900 mb-4">Revenue Breakdown (This Month)</h2>
            <div className="space-y-4">
              {[
                { category: 'Consultations', amount: 125000, percentage: 27, color: 'bg-hospital-purple' },
                { category: 'Laboratory', amount: 98000, percentage: 21, color: 'bg-teal-500' },
                { category: 'Pharmacy', amount: 112000, percentage: 25, color: 'bg-blue-500' },
                { category: 'Radiology', amount: 67000, percentage: 15, color: 'bg-orange-500' },
                { category: 'Procedures', amount: 54320, percentage: 12, color: 'bg-success-500' },
              ].map((item, i) => (
                <div key={i}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white/60 backdrop-blur-md rounded-xl shadow-soft p-6 border border-gray-100">
          <h2 className="text-xl font-bold text-gray-900 mb-4">Top Earning Doctors (MTD)</h2>
          <div className="space-y-3">
            {[
              ...
              <div key={i} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                ...
                <span className="font-bold text-success-600">${doctor.revenue.toLocaleString()}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white/60 backdrop-blur-md rounded-xl shadow-soft p-6 border border-gray-100">
          <h2 className="text-xl font-bold text-gray-900 mb-4">Insurance Claims Status</h2>
          <div className="space-y-3">
            {[
              { status: 'Pending', count: 23, amount: 45600, color: 'bg-warning-100 text-warning-700' },
              { status: 'Approved', count: 45, amount: 89000, color: 'bg-success-100 text-success-700' },
              { status: 'Rejected', count: 5, amount: 8900, color: 'bg-error-100 text-error-700' },
              { status: 'Under Review', count: 12, amount: 23400, color: 'bg-blue-100 text-blue-700' },
            ].map((claim, i) => (
