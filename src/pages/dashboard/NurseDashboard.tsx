...
import { Activity, Users, Pill, FileText, AlertCircle, Thermometer, Calendar, Clock, CheckCircle } from 'lucide-react';
...
export function NurseDashboard() {
  ...
  const handleViewMedications = async () => {
    ...
    try {
      ...
      setPrescriptions(data.filter((p: any) => p.status === 'PENDING') || []);
    } catch (error) {
  ...
  const handleCheckInPatient = async (id: string) => {
    try {
  ...
  const ipdPatients = patients.filter(p => p.status === 'IPD' || p.status === 'EMERGENCY');
  const opdPatients = patients.filter(p => p.status === 'OPD');
  const pendingMeds = prescriptions.filter(p => p.status === 'PENDING').length;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-display font-bold text-gray-900">Nurse Station</h1>
          <p className="text-gray-600 mt-1">Patient care and monitoring</p>
        </div>
        <div className="flex gap-3">
          <Button variant="outline" icon={Thermometer} onClick={() => handleViewPatients()}>
            Record Vitals
          </Button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2">
          <div className="bg-white/60 backdrop-blur-md rounded-xl shadow-soft p-6 border border-gray-100">
            <h2 className="text-xl font-bold text-gray-900 mb-4">IPD Patients</h2>
            {loading ? (
        ...
        {selectedPatient && (
          <div className="space-y-4">
            <div className="bg-blue-50 p-4 rounded-lg">
              ...
              <p className="text-sm text-gray-600">UPID: {selectedPatient.upid}</p>
            </div>
          ...
          {loadingMedications ? (
            ...
            <DataTable
              ...
              columns={[
                ...
                {
                  header: 'Patient',
                  accessor: (row) => row.patient ? `${row.patient.first_name} ${row.patient.last_name}` : 'N/A'
                },
                {
                  header: 'Date',
                  accessor: (row) => new Date(row.prescribed_date).toLocaleDateString()
                },
                {
                  header: 'Status',
                  accessor: (row) => (
          ...
          {loadingAppointments ? (
            ...
            <DataTable
              ...
              columns={[
                ...
                {
                  header: 'Time',
                  accessor: (row) => new Date(row.scheduled_at).toLocaleTimeString()
                },
                {
                  header: 'Status',
                  accessor: (row) => (
